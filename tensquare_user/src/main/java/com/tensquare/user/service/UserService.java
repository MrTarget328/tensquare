...
import com.tensquare.user.controller.BaseExceptionHandler;
...
public class UserService {
	...
	public void deleteById(String id) {
		Claims token = (Claims) request.getAttribute("admin_claims");
		if (token == null){
    ...
    public void sendSms(String moblie) {
		...
		//2.存入Redis
		redisTemplate.opsForValue().set("checkCode_"+moblie,checkCode);
		//3.通过rabbitmq发送消息
		Map<String,String> map = new HashMap<>();
		//验证码
		Map<String,String> mapCode = new HashMap<>();
		mapCode.put("name",checkCode);
		String code = JSON.toJSONString(mapCode);
		map.put("checkCode",code);
		//手机号
		map.put("moblie",moblie);
		//模板
		map.put("template_code","SMS_172885309");
		//签名
		map.put("sign_name","MrLiu");
    ...
    public void addNum(String friendId, String userId,int x) {
		//更新粉丝数
